...
Vtester.prototype.build = function() {
    ...
    ucArray.forEach((uc, index) => {
        ...
        if (uc.build === undefined || uc.build === true) {
            ...
            if (this.vtestConfig.platform == 'android' || this.vtestConfig.platform == 'ios') {
                ucFileNameMap[uc.ucKey] = `
                    if('${fileName.replace(".uc.js","")}'===uc || uc===undefined){
                        require('./${fileName}')(driver,router);
                    }
                `;
    ...
    logger.debug('----------build method start----------');
    let methodList = [];
    let config = _.extend({}, { vtestConfig: this.vtestConfig });
    Object.keys(this.plugins.method).forEach(key=>{
        logger.debug(`build ${key}`);
        let methodPlugin = this.plugins.method[key];
        methodList.push({
            name:key,
            func:methodPlugin.build(config)
        });
    });
    logger.debug('----------build method end----------');
    //处理driver扩展
